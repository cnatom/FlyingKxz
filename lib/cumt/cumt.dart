...
class Cumt {
  ...
  Future<void> init() async {
    ...
    dio.interceptors.add(new CookieManager(
      cookieJar,
    ));
    isLogin = false;
  }
  ...
  Future<bool> loginCheckGet(BuildContext context,
    ...
    try {
      ...
      var res = await dio.get(
        "http://authserver.cumt.edu.cn/authserver/checkNeedCaptcha.htl",
      ...
      if (map['isNeedActive'] != null && map['isNeedActive']) {
        ...
        Future.delayed(Duration(seconds: 3), () {
          launchUrl(Uri.parse(
              "http://authserver.cumt.edu.cn/retrieve-password/accountActivation/index.html"));
        });
  ...
  Future<String> loginYkt() async {
    try {
      await loginDefault();
      var url =
          "https://yktm.cumt.edu.cn/berserker-auth/cas/login/wisedu?targetUrl=https%3A//yktm.cumt.edu.cn/plat-pc/%3Fname%3DloginTransit";
      Response res;
      // 循环重定向
      while (true) {
  ...
  Future<bool> login(String username, String password) async {
    ...
    try {
      ...
      var res = await dio.get(
          'https://authserver.cumt.edu.cn/authserver/login?service=$service',
          options: Options(
      ...
      if (res.toString().length > 35000) {
        ...
        var pwdSalt = document.body
                ?.querySelector("#pwdEncryptSalt")
                ?.attributes['value'] ??
            '';
        var execution = document.body
                ?.querySelectorAll('#execution')[2]
                .attributes['value'] ??
            '';
        ...
        var res2 = await dio.get(loginResponse.headers.value('location')!,
            options: Options(followRedirects: false));
        isLogin = true;
  ...
  Future<bool> loginFWDT(String username, String password) async {
    try {
      ...
      var ssoticketid = document2.body
              ?.querySelector("input[id='ssoticketid']")
              ?.attributes['value'] ??
          '';
      var res6 = await dio.post("http://ykt.cumt.edu.cn/cassyno/index",
          data: FormData.fromMap({
            "errorcode": "1",
            "continueurl": "",
  ...
  static Future<bool> checkConnect(
      {bool showToasts = true, int timeout = 4}) async {
    try {
      ...
      Response res = await Dio(BaseOptions(
          ...
          .get('http://jwxt.cumt.edu.cn/jwglxt/xtgl/login_slogin.html');
  ...
  Future<Map<String, dynamic>> getNamePhone() async {
    ...
    try {
      var res = await dio
          .get('http://portal.cumt.edu.cn/portal/api/v2/infoplus/me/profile');
      ...
      result = {'name': map['name'] ?? '', 'phone': map['phone'] ?? ''};
    } catch (e) {
      result = {'name': '', 'phone': ''};
    }
    return result;
  ...
  Future<String> _pwdAes(String password, String salt) async {
    try {
      ...
      try {
        ...
        response = await Dio().get('https://aes.atcumt.com/password',
