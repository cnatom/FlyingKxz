...
class ImportPage extends StatefulWidget {
  final ImportCourseType importType;

  const ImportPage({Key? key, required this.importType}) : super(key: key);
...
class _ImportPageState extends State<ImportPage> {
  ...
  init() async {
    if (widget.importType == ImportCourseType.BK &&
        !await Cumt.checkConnect()) {
      toTipPage();
    }
  }
  ...
  _import() async {
    ...
    if (list == null || list.isEmpty) {
      showToast('未找到课程信息，请确保当前为课表页，且更新至最新版本');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return FlyWebViewInApp(
        ...
        actions: [
          FlyWebViewAction(iconData: Boxicons.bx_help_circle,onPressed: (){
            Navigator.of(context).push(CupertinoPageRoute(
                builder: (context) => ImportHelpPage(
        ...
        onLoadStop: (controller, url) async {
          ...
          if (widget.importType == ImportCourseType.YJS) {
            if (url.rawValue.contains("http://yjsxt.cumt.edu.cn/Gstudent/Default.aspx")) {
              ...
              await _controller.evaluateJavascript(source: "document.getElementById('MenuFrame').contentDocument.querySelector('#tree1_4_a').click();");
            }
          }
        },
        stackChildren: [
    ...
    return Scaffold(
            ...
            IconButton(
                ...
                onPressed: () async {
                  ...
                  await _controller.loadUrl(
                      urlRequest:
                          URLRequest(url: WebUri(urlMap[widget.importType]!)));
                }),
            IconButton(
                icon: Icon(
      ...
      body: Stack(
        ...
        children: [
          ...
          //       showToast(
          //           "正在自动跳转到课表页面......请耐心等待，若我自动消失后还没出现课表页，可按下方[导入步骤]手动跳转",
          //           duration: 15);
          //       await _controller.zoomBy(zoomFactor: 2, animated: true);
          //       await _controller.scrollTo(x: 0, y: 0, animated: true);
          ...
          //
          //     }
          //     if (url.toString().contains("http://jwxt.cumt.edu.cn/jwglxt/kbcx/xskbcx")){
          //       if(Prefs.passwordJw == null){
          //         bool? confirm =  await showDialogConfirm(context, title: "是否记住当前密码，下次自动填充此密码？",onConfirm: (){
          //
          //         });
          //         if (confirm==true){
          //
...
class ImportHelpPage extends StatefulWidget {
  ...
  ImportHelpPage({required this.type});
